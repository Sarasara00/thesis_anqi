...
import Levenshtein
def determine_final_output(network,numeric_dict):
    pattern1=r"\[(.*?)\],"
    pattern2=r"\[(.*?)\] ;"
    matches1 = re.findall(pattern1, network)
    network=remove_substring(network,matches1)
    ...
    for match in all_matches:
        tool_match = re.search(r"<https://quangis.github.io/tool/abstract#(\w+)", match)
        input_matches = re.findall(r"ns1:input\w+ _:(\w+)", match)
        output_match = re.search(r"ns1:output _:(\w+)", match)
        if output_match==None:
            final_output_tool_name = tool_match.group(1)
...
def extract_nodes_and_connections(network):
    ...
    for match in all_matches:
        ...
        output_match = re.search(r"ns1:output _:(\w+)", match)
        if output_match==None:
            output_match=re.search(r'rdfs:label "(.*?)"', match)
        if tool_match and input_matches and output_match:
            ...
            for node in nodes:
                for input1 in input_nodes:
                    ...
                    if input_node1 in node and combined_node!=node:
...
def extract_ccd(network):
    ...
    for match in matches:
        data_name_match = re.search(r"(\w+)\sa\s", match)
        ccd_match = re.search(r'rdfs:label "(.*?)"', match)
        #output_match = re.search(r"wf:output _:(\w+)", match)

        if data_name_match and ccd_match:
            data_name_match_group=data_name_match.group(1)
            data_names.append(data_name_match_group)
            combined_ccd=ccd_match.group(1)
...
def replace_data_type(node,combined_ccds,data_names):
    ...
    if not output_name.startswith('N'):
        output_type=re.findall(r"\+.+$",node)
...
def replace_commas(numeric_dict):
    numeric_dict=numeric_dict.replace(',','_').replace(' ','')
    return numeric_dict
...
def extract_labels(connections, start):
    ...
    return labels
#read ttl.file
file_path_1 = '../data_source/wfgen/solution100.ttl'
...
print('connections:',connections_1)
print('nodes:',nodes_1)
...
print('converted_nested_list:',converted_nested_list)
numeric_dict={key:replace_commas(value) for key,value in numeric_dict.items()}
print('numeric_dict:',numeric_dict)
final_tool=determine_final_output(content_1,numeric_dict)
...
input_folder = os.path.join(current_dir, "../data_source/wfgen/")
output_folder = os.path.join(current_dir, "../data_source/json_file_generated/")
...
for file in os.listdir(input_folder):
    if file.endswith('.ttl'):
        ...
        numeric_dict = {key: replace_commas(value) for key, value in numeric_dict.items()}
        final_tool = determine_final_output(content, numeric_dict)
        start_node = Node(final_tool)
        labels = extract_labels(converted_nested_list, start_node)
        converted_nested_list, numeric_dict_new = change_diagram_order(labels, numeric_dict, converted_nested_list)
        # 创建JSON数据
        ...
        # 生成JSON文件
        numbers = re.findall(r'solution(\d+)', os.path.basename(file_name))
